 ...
 *
 * Catatan: endpoint "watch" versi shortdramavid.xyz TIDAK ada di API asli
 * ini (`invalid action "watch"`) — aksi yang benar untuk resolve stream
 * adalah `hls`. Endpoint "notifications" juga tidak ada di API asli;
 * fitur itu murni buatan shortdramavid.xyz jadi kita fallback ke sana
 * hanya untuk notifications (opsional, gagal pun tidak masalah).
 */

const { fetchJSON } = require("../fetcher");

const BASE = "https://priv-api.anichin.bio/api";
const NOTIF_FALLBACK_BASE = "https://www.shortdramavid.xyz";
...
/**
 * Status platform — TIDAK ada di API asli, fallback opsional ke
 * shortdramavid.xyz (kalau gagal, kembalikan array kosong saja).
 */
async function notifications() {
  try {
    const raw = await fetchJSON(`${NOTIF_FALLBACK_BASE}/api/notifications/`);
