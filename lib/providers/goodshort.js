...
/**
 * Wrapper fetchJSON yang menangkap error maintenance upstream secara graceful.
 * Saat GoodShort sedang maintenance, upstream mengembalikan HTTP 4xx/error —
 * daripada throw ke client (menyebabkan 500), kembalikan null supaya tiap
 * fungsi bisa memutuskan fallback yang tepat (biasanya array kosong).
 */
...
async function languages() {
  const raw = await safeFetch(buildUrl("languages"));
...
async function trending(provider, lang) {
  const raw  = await safeFetch(buildUrl("trending", {}, lang));
  if (!raw) return [];
...
async function latest(provider, lang) {
...
async function browse(provider, lang) {
...
async function foryou(provider, page = 1, lang) {
  const raw  = await safeFetch(buildUrl("foryou", { page: Number(page) || 1 }, lang));
 ...
 */
async function search(q, provider, lang) {
  if (!q || q.trim().length < 2) return [];
  const raw  = await safeFetch(buildUrl("search", { query: q.trim(), page: 1 }, lang));
  if (!raw) return [];
  const list = Array.isArray(raw) ? raw : (raw.items ?? []);
...
async function allepisode(provider, id) {
  const raw      = await safeFetch(buildUrl("allepisode", { id }));
...
async function detail(provider, id) {
  ...
  const [info, eps] = await Promise.all([
    safeFetch(buildUrl("detail", { id })),
    allepisode(provider, id).catch(() => ({ episodes: [], totalEpisodes: 0, bookName: "", cover: "" })),
  ]);

  if (!info) {
    return {
      id, title: eps.bookName || "Tanpa Judul", cover: eps.cover || "",
      description: "", totalEpisodes: eps.totalEpisodes || eps.episodes.length,
      episodes: eps.episodes, provider,
    };
  }
...
async function stream(provider, id, ep = 1) {
  const epData = await safeFetch(buildUrl("episode", { id, ep: Number(ep) }));

  if (!epData || epData.locked) {
    return {
      videoUrl:      "",
      locked:        true,
