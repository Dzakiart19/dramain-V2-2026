...
async function trending(provider) {
  const raw  = await safeFetch(buildUrl("trending"));
  if (!raw) return [];
...
async function foryou(provider, page = 1) {
  const raw  = await safeFetch(buildUrl("foryou", { page: Number(page) || 1 }));
...
async function search(q, provider) {
  ...
  const raw  = await safeFetch(buildUrl("search", { query: q.trim(), page: 1 }));
  if (!raw) return [];
  const list = Array.isArray(raw) ? raw : (raw.items ?? []);
