...
function buildUrl(action, params = {}, lang = DEFAULT_LANG) {
...
async function trending(provider, lang) {
  const raw  = await fetchJSON(buildUrl("trending", {}, lang), { headers: authHeaders() });
...
async function foryou(provider, page = 1, lang) {
  const raw  = await fetchJSON(buildUrl("foryou", { page: Number(page) || 1 }, lang), { headers: authHeaders() });
  const list = Array.isArray(raw) ? raw : (raw.items ?? []);
  return {
...
async function search(q, provider, lang) {
  ...
  const raw  = await fetchJSON(buildUrl("search", { query: q.trim() }, lang), { headers: authHeaders() });
  const list = Array.isArray(raw) ? raw : (raw.items ?? []);
