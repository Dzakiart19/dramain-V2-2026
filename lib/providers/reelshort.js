...
function get(action, params = {}, lang) {
  return fetchJSON(buildUrl(action, params, lang), { headers: authHeaders() });
}
...
async function trending(provider, lang) {
  const raw  = await get("trending", {}, lang);
...
async function foryou(provider, page = 1, lang) {
  const raw  = await get("foryou", { page: Number(page) || 1 }, lang);
  const list = Array.isArray(raw) ? raw : (raw.items ?? []);
  return {
...
async function search(q, provider, lang) {
  ...
  const raw  = await get("search", { query: q.trim() }, lang);
  const list = Array.isArray(raw) ? raw : (raw.items ?? []);
