...
/** Buat URL endpoint, tambah lang. Key dikirim via header, bukan URL. */
function buildUrl(action, params = {}, lang = LANG) {
  const qs = new URLSearchParams({ lang, ...params });
...
async function trending(provider, lang) {
  const raw  = await fetchJSON(buildUrl("trending", {}, lang), authOpts());
  const list = Array.isArray(raw) ? raw : (raw.items ?? []);
  return list.map((d) => normalizeItem(d, provider));
}

async function latest(provider, lang) {
  const raw  = await fetchJSON(buildUrl("latest", {}, lang), authOpts());
  const list = Array.isArray(raw) ? raw : (raw.items ?? []);
  return list.map((d) => normalizeItem(d, provider));
}

async function foryou(provider, page = 1, lang) {
  const raw  = await fetchJSON(buildUrl("foryou", { page: Number(page) || 1 }, lang), authOpts());
  const list = Array.isArray(raw) ? raw : (raw.items ?? []);
  return {
...
async function search(q, provider, lang) {
  ...
  const raw  = await fetchJSON(buildUrl("search", { q }, lang), authOpts());
  const list = Array.isArray(raw) ? raw : (raw.items ?? []);
  return list.map((d) => normalizeItem(d, provider));
}
